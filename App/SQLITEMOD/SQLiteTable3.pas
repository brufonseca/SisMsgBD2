...

{
  ...
  SQL parameter usage:
   ...
   1. Call PrepareSQL to return TSQLiteQuery
   2. Call BindSQL for each parameter in the prepared query
   3. Call ExecSQL to run the prepared query
   4. Repeat steps 2 & 3 as required
   5. Call ReleaseSQL to free SQLite resources
...
uses
  ...
  Windows,
  {$ENDIF}
  SQLite3  {esta contem o tipo TSQLiteDB} , Classes, SysUtils, DIALOGS;
...
// --------------- funcoes para uso de libs --------------------------------
// ***** USO DE BIB EXTERNA DE FUNCOES (ex. libsqlitefunctions.so ja pronta)

//FOI MODIFICADA A UNIT sqlitetable3

 //1) foram acrescentadas na INTERFACE de sqliteTable3
//
function sqlite3_enable_load_extension(db: TSQLiteDB; onoff:integer):integer;cdecl;
 external SQLiteDLL name 'sqlite3_enable_load_extension';
function sqlite3_load_extension(db: TSQLiteDB;nomelib:string; parm3:integer):integer;cdecl;
 external SQLiteDLL name 'sqlite3_load_extension';
const lib='..\..\app\bd_iu\libsqlitefunctions.so';
// corrigir esta const para o lugar da biblioteca atual
// Assim neste caso a lib esta em
// uma pasta APP\bd_IU3paralela a este sistema aqui SISxxx
// essa pasta tem sqlite3,sqlitetable3,bd_iu3.pas etc
// (senao da erro ao usarmos as funcoes)

// Usando essa lib no nomelib com parm3=0 poemos usar funcoes COSH, CHARINDEX ETC
//  exemplo: sqlite3_load_extension(db,    lib,0) ;
// usa db do tipo TsqliteDB que estah na Unit Sqlite3

//2) e foram acrescentadas no modulo TSQLiteDatabase.Create
 //  as chamadas correspondentes  (ver adiante)

// ------------------------------------------------------------------
//  sqlite3_enable_load_extension(fdb,1) ;  // PERMITE FUNCAO EXTERNA  ...
//  sqlite3_load_extension(fdb,lib,0) ; // lib eh a lib de funcoes ext atual
// ------------------------------------------------------------------
// -------------------fim MUDANCAS --------------------------
...
// ------------------------------------------------------------------
//  --------- ** USO DE FUNCOES para extender com uso de BIB EXTERNA ** ------
    sqlite3_enable_load_extension(fdb,1) ;  // PERMITE FUNCAO EXT perigoso ...
    sqlite3_load_extension(fdb,lib,0) ; // lib eh a lib de funcoes ext atual
// ------------------------------------------------------------------
...
begin
 // SHOWMESSAGE(' TIPO '+INTTOSTR(I)+' '+INTTOSTR(pInteger(self.fColTypes[I])^) );
  if EOF then
    raise ESqliteException.Create('Table is at End of File');
  if (self.fResults[(self.frow * self.fColCount) + I] = nil) then
    Result := 0
